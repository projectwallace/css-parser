...
import type { DeclarationNode } from './node-types'
 ...
 */
export function parse_declaration(source: string): DeclarationNode {
	...
	if (decl_index === null) {
		...
		return new CSSNode(arena, source, empty) as DeclarationNode
	}

	// Wrap in CSSNode
	return new CSSNode(arena, source, decl_index) as DeclarationNode
}
