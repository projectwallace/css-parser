...
import type { Number, Dimension, Function } from './node-types'
...
describe('parse_declaration', () => {
	...
	describe('browser hacks', () => {
		...
		test('value\\9', () => {
			...
			expect(node.first_child!.text).toBe('value\\9')
	...
	describe('Value Parsing', () => {
		...
		test('number value', () => {
			...
			expect((node.first_child!.children[0] as Number).value).toBe(0.5)
		})
		...
		test('dimension value', () => {
			...
			expect((node.first_child!.children[0] as Dimension).value).toBe(100)
			expect((node.first_child!.children[0] as Dimension).unit).toBe('px')
		})
		...
		test('function value', () => {
			...
			expect((node.first_child!.children[0] as Function).name).toBe('rotate')
		})
		...
		test('nested functions', () => {
			...
			expect(node.first_child!.children).toHaveLength(1)
			expect(node.first_child!.children[0].type).toBe(FUNCTION)
			expect((node.first_child!.children[0] as Function).name).toBe('calc')
		...
		test('complex value with multiple functions', () => {
			...
			expect(node.first_child!.children).toHaveLength(1)
			expect(node.first_child!.children[0].type).toBe(FUNCTION)
			expect((node.first_child!.children[0] as Function).name).toBe('linear-gradient')
		})
		...
		test('CSS variable', () => {
			...
			expect(node.first_child!.children).toHaveLength(1)
			expect(node.first_child!.children[0].type).toBe(FUNCTION)
			expect((node.first_child!.children[0] as Function).name).toBe('var')
		})
	})
	...
	describe('CSSNode API', () => {
		...
		test('node.value returns raw value string', () => {
			...
			expect(node.first_child!.text).toBe('10px 20px')
		})
